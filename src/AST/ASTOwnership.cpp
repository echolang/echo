...
ExprNode *OwnershipPass::arrive_computed_place(ExprNode *expr)
{
    ...
    // t_materializable form, so arrive_value of it never inserts the copy. hang the
    // copy on the producing place. read_reaches_storage so a T&-returning call is
    // copied too; resolve_value_arrival so walking that call first would not arrive
    // its arguments twice
...
ExprNode *OwnershipPass::walk_expression(ExprNode *expr)
{
    ...
    switch (expr->get_node_type()) {
        ...
        case NodeType::n_expr_optional_chain:
        {
            ...
            // holding it - so it cannot ask for the base a second time. a place continuation
            // copies on the present path; a computed one is already an owner
        ...
        case NodeType::n_expr_match:
        {
            ...
            for (MatchExprNode::Arm &arm : node->arms) {
                ...
                // mixed arms: the match is t_materializable, so each place arm copies here.
                // all-place stays a walk - the match itself is the address
...
)
{
    ...
    //
    // a place yielded by a t_materializable form is not this arrival's to copy.
    // arrive_computed_place hung it on the producing edge; a place lhs of `??`
    // hangs its own off present_value. by the time the form arrives here it is
    // already a copy nobody else holds
    //
