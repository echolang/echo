...
void Parser::parse_type_names(Parser::Payload &payload)
{
    ...
    while (!cursor.is_done())
    {
        ...
        if (!cursor.is_type(Token::Type::t_struct) || !cursor.peek_is_type(1, Token::Type::t_identifier)) {
        ...
        cursor.skip(); // the struct keyword
        ...
        // codegen two StructDeclNodes for one type. parse_struct then reuses whichever node is here.
        // the duplicate is left symbol-less deliberately and reported nowhere here: both later passes
        // then find the *first* node and parse_struct reports the redeclaration at the duplicate's own
        // name token, which is also where the body-skip recovery lives. this pass has no such
        // recovery, and its detection is a strict subset of parse_struct's anyway
        auto *existing = payload.collector.namespaces.find_symbol(name_token.value(), *payload.context.current_namespace);
        if (existing != nullptr && existing->node.get_ptr<AST::StructDeclNode>() != nullptr) {
            continue;
        }

        auto &struct_node = payload.context.emplace_node<AST::StructDeclNode>(name_token);
        struct_node.set_namespace(payload.context.current_namespace);

        payload.context.current_namespace->push_symbol(std::make_unique<AST::Symbol>(&struct_node));
    }
}
